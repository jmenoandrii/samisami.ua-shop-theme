...
<div class="product__variants js-product-variants">
  {foreach from=$groups key=id_attribute_group item=group}
    ...
    <div class="variant">
      <label for="group_{$id_attribute_group}" class="form-label">{$group.name}</label>
          ...
          {foreach from=$group.attributes key=id_attribute item=group_attribute}
            <li class="color-variant {if $group_attribute.selected}active{/if}">
              <label aria-label="{$group_attribute.name}">
                ...
                <span class="color texture {if $group_attribute.selected}active{/if}" style="background-color: {$group_attribute.html_color_code}; {if $group_attribute.texture}background-image: url({$group_attribute.texture}){/if}"
